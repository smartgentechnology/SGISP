...
class NewPageTool{
	...
	public function show($pageflag='page'){
		...
		$nav=array();
		$nav[0]='<li><a class="active" href="#">'.$this->page.'</a></li>';
		
		for($left=$this->page-1,$right=$this->page+1;($left>=1||$right<=$cnt)&& count($nav)<5;){
				if($left>=1){
				array_unshift($nav,'<li><a href="'.$url.$pageflag.'='.$left.'">['.$left.']</a></li>');
				$left-=1;
				}
				if($right<=$cnt){
				array_push($nav,'<li><a href="'.$url.$pageflag.'='.$right.'">['.$right.']</a></li>');
				$right+=1;
				}
