...
class QiankuanController extends AuthController {
	
    public function index(){
		$this->assign('pagetitle','客户欠款');
		$this->display();
    }
	public function getqiankuan(){
		//得到查询条件
		//截止日期
		$month=I('post.month');
		//拼接完成日期
		if(!empty($month)){
			$fahuotime=strtotime($month);
		}else{
			$fahuotime=time();
		}
		session('qiankuantime',$fahuotime);
		$qiankuan=new \lib\Qiankuan();
		$datalist=$qiankuan->kehu($fahuotime);
		if(!empty($datalist)){
	...
	public function exportExcel(){
		$xlsTitle = iconv('utf-8', 'gb2312', '客户欠款');//文件名称
		$expCellName  = array(
		array('tian','日期'),
		array('count','客户欠款')
		);
		...
		$fahuotime=session('qiankuantime');
		if(empty($fahuotime)){
			$fahuotime=time();
		}
		$qiankuan=new \lib\Qiankuan();
		$datalist=$qiankuan->export($fahuotime);
