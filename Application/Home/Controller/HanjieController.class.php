...
class HanjieController extends AuthController {
	...
	public function index(){
		...
		//得到焊接工序的数据
		$hanjielist=$model->where("batch='%s' and type=%d",$batch,1)->order('id desc')->select();
		//调用工具类
		$utils=new \lib\Utils();
		//焊接数据格式化
		$hanjielist=$utils->changeTime($hanjielist);
		$hanjielist=$utils->managerIdtoName($hanjielist);
		$hanjielist=$utils->flagtoState($hanjielist);
		$this->assign('pagetitle','生产计划详细信息');
		$this->assign('batchinfo',$batchinfo);
		$this->assign('hanjielist',$hanjielist);
		$this->display();
    }
	...
	public function add(){
		...
		//焊接报废的
		$scrapnumber=$model->where("batch='%s' and type=%d and flag=%d",$data['batch'],1,5)->field('SUM(number) as total')->select();
		$scrapnumber=$scrapnumber[0]['total']+0;
		//焊接完成的
		$completenumber=$model->where("batch='%s' and type=%d and flag=%d",$data['batch'],1,1)->field('SUM(number) as total')->select();
		$completenumber=$completenumber[0]['total']+0;
		$total=$batchnumber-$scrapnumber;
		if($total<=$completenumber){
			$this->ajaxReturn(array(
				'state'=>'error',
				'info' => '本批次数量'.$batchnumber.'台，焊接报废'.$scrapnumber.'台，已完成'.$completenumber.'台，不能添加！'
			));
		}else{
			$flaga=$model->where("batch='%s' and type=%d and flag=%d",$data['batch'],1,0)->find();
			if($flaga){
				$this->ajaxReturn(array(
					'state'=>'error',
					'info' => '已有未完成记录！'
	...
	public function edit(){
		...
		if(IS_GET){
			...
			$batch=I('post.batch');
			//批次数量
			$batchnumber=$pschedule->where("batch='%s'",$batch)->field('number')->find();
			$batchnumber=$batchnumber['number']+0;
			...
			if($buttonid!=3){
				//焊接报废的
				$scrapnumber=$model->where("batch='%s' and type=%d and flag=%d",$batch,1,5)->field('SUM(number) as total')->select();
				$scrapnumber=$scrapnumber[0]['total']+0;
				//焊接完成的
				$completenumber=$model->where("batch='%s' and type=%d and flag=%d",$batch,1,1)->field('SUM(number) as total')->select();
				$completenumber=$completenumber[0]['total']+0;
				if($buttonid==1 || $buttonid==2){
					...
					if($total<$addnumber){
						$this->ajaxReturn(array(
							'state'=>'error',
							'info' => '本批次的数量'.$batchnumber.'台，焊接报废'.$scrapnumber.'台，本工序已完成'.$completenumber.'台，不能修改！'
						));
					}
				}else if($buttonid==4){
					//报废+新数量
					...
					//总数-已完成数量
					$total=$batchnumber-$completenumber;
					if($total<$addnumber){
						$this->ajaxReturn(array(
							'state'=>'error',
							'info' => '本批次的数量'.$batchnumber.'台，焊接已完成'.$completenumber.'台，已报废'.$scrapnumber.'台，不能修改！'
						));
					}
				}
				...
				//焊接报废除了修改ID之外的数量
				$scrapnumber=$model->where("batch='%s' and type=%d and flag=%d and id !=%d",$batch,1,5,$id)->field('SUM(number) as total')->select();
				$scrapnumber=$scrapnumber[0]['total']+0;
				//焊接完成除了修改ID之外的数量
				$completenumber=$model->where("batch='%s' and type=%d and flag=%d and id !=%d",$batch,1,1,$id)->field('SUM(number) as total')->select();
				$completenumber=$completenumber[0]['total']+0;
			...
			if (!$model->create($data)){ // 创建数据对象
				...
				if($flagb!==false){
					//焊接报废总数量
					$scraptotal=$model->where("batch='%s' and type=%d and flag=%d",$batch,1,5)->sum('number');
					$scraptotal=$scraptotal+0;
					//焊接报废入库数量
					$scrapdailiao=$model->where("batch='%s' and type=%d and flag=%d",$batch,8,7)->sum('number');
					...
					if($scrapdailiao==0){
						...
						$model->where("batch='%s' and type=%d and flag=%d",$batch,8,7)->save();
					}
					//更改计划状态和在线上在线数量
