...
class ChushiController extends AuthController {
	...
	public function index(){
		...
		//得到初试工序的数据
		$chushilist=$model->where("batch='%s' and type=%d",$batch,2)->order('id desc')->select();
		//调用工具类
		$utils=new \lib\Utils();
		//初试数据格式化
		$chushilist=$utils->changeTime($chushilist);
		$chushilist=$utils->managerIdtoName($chushilist);
		$chushilist=$utils->flagtoState($chushilist);
		$this->assign('pagetitle','生产计划详细信息');
		$this->assign('batchinfo',$batchinfo);
		$this->assign('chushilist',$chushilist);
		$this->display();
    }
	...
	public function add(){
		...
		//初试报废的
		$scrapnumber=$model->where("batch='%s' and type=%d and flag=%d",$data['batch'],2,5)->field('SUM(number) as total')->select();
		$scrapnumber=$scrapnumber[0]['total']+0;
		//初试完成的
		$completenumber=$model->where("batch='%s' and type=%d and flag=%d",$data['batch'],2,1)->field('SUM(number) as total')->select();
		$completenumber=$completenumber[0]['total']+0;
		$total=$batchnumber-$scrapnumber;
		if($total<=$completenumber){
			$this->ajaxReturn(array(
				'state'=>'error',
				'info' => '本批次数量'.$batchnumber.'台，初试报废'.$scrapnumber.'台，已完成'.$completenumber.'台，不能添加！'
			));
		}else{
		
			$flaga=$model->where("batch='%s' and type=%d and flag=%d",$data['batch'],2,0)->find();
			if($flaga){
				$this->ajaxReturn(array(
					'state'=>'error',
					'info' => '已有未完成记录！'
		...
		/*
		//上一道工序完成的数量
		$batchnumber=$model->where("batch='%s' and type=%d and flag=%d",$data['batch'],1,1)->field('SUM(number) as total')->select();
		$batchnumber=$batchnumber[0]['total']+0;
		//焊接入库数量
		$hanjietotal=$model->where("batch='%s' and type=%d and flag=%d",$data['batch'],6,2)->field('SUM(number) as total')->select();
		...
		if($batchnumber==0){
			...
			$total=$batchnumber-$hanjietotal-$scrapnumber;
			if($total<=$completenumber){
				$this->ajaxReturn(array(
					'state'=>'error',
					'info' => '焊接完成数量'.$batchnumber.'台，入库'.$hanjietotal.'台，本工序已报废'.$scrapnumber.'台，已完成'.$completenumber.'台，不能添加！'
				));
			}else{
				$flaga=$model->where("batch='%s' and type=%d and flag=%d",$data['batch'],2,0)->find();
				if($flaga){
	...
	public function edit(){
		...
		if(IS_GET){
			...
			$batch=I('post.batch');
			//焊接完成的数量
			$batchnumber=$model->where("batch='%s' and type=%d and flag=%d",$batch,1,1)->field('SUM(number) as total')->select();
			$batchnumber=$batchnumber[0]['total']+0;
			//焊接入库数量
			$hanjietotal=$model->where("batch='%s' and type=%d and flag=%d",$batch,6,2)->field('SUM(number) as total')->select();
			$hanjietotal=$hanjietotal[0]['total']+0;
			...
			if($buttonid!=3){
				//初试报废的
				$scrapnumber=$model->where("batch='%s' and type=%d and flag=%d",$batch,2,5)->field('SUM(number) as total')->select();
				$scrapnumber=$scrapnumber[0]['total']+0;
				//本工序已完成的数量
				$completenumber=$model->where("batch='%s' and type=%d and flag=%d",$batch,2,1)->field('SUM(number) as total')->select();
				$completenumber=$completenumber[0]['total']+0;
				if($buttonid==1 || $buttonid==2){
					//已完成+新数量
					$addnumber=$completenumber+$addnumber;
					//总数-报废数量-上一工序入库数量
					$total=$batchnumber-$scrapnumber-$hanjietotal;
					if($total<$addnumber){
						$this->ajaxReturn(array(
							'state'=>'error',
							'info' => '焊接完成数量'.$batchnumber.'台，入库'.$hanjietotal.'台，本工序已报废'.$scrapnumber.'台，已完成'.$completenumber.'台，不能修改！'
						));
					}
				}else if($buttonid==4){
					//报废+新数量
					...
					//总数-已完成数量-上一工序入库数量
					$total=$batchnumber-$completenumber-$hanjietotal;
					if($total<$addnumber){
						$this->ajaxReturn(array(
							'state'=>'error',
							'info' => '焊接完成数量'.$batchnumber.'台，入库'.$hanjietotal.'台，本工序已完成'.$completenumber.'台，已报废'.$scrapnumber.'台，不能修改！'
						));
					}
				}
				...
				//初试报废的
				$scrapnumber=$model->where("batch='%s' and type=%d and flag=%d and id!=%d",$batch,2,5,$id)->field('SUM(number) as total')->select();
				$scrapnumber=$scrapnumber[0]['total']+0;
				//本工序已完成的数量
				$completenumber=$model->where("batch='%s' and type=%d and flag=%d and id!=%d",$batch,2,1,$id)->field('SUM(number) as total')->select();
				$completenumber=$completenumber[0]['total']+0;
			...
			if (!$model->create($data)){ // 创建数据对象
				...
				if($flagb!==false){
					//初试报废总数量
					$scraptotal=$model->where("batch='%s' and type=%d and flag=%d",$batch,2,5)->sum('number');
					$scraptotal=$scraptotal+0;
					//初试报废入库数量
					$scrapdailiao=$model->where("batch='%s' and type=%d and flag=%d",$batch,8,8)->sum('number');
					...
					if($scrapdailiao==0){
						...
						$model->where("batch='%s' and type=%d and flag=%d",$batch,8,8)->save();
					}
					//更改计划状态和在线上在线数量
