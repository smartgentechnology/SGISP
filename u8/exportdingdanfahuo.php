...
//print_r($bumanzulist);
$datalist=array_merge($manzulist,$bumanzulist);
//print_r($datalist);
$str = "订单号,客户,审核,日期,型号,应发,已发,库存,备注";
$exl11 = explode(',',$str);

exportToExcel("dingdanfahuo.xls",$exl11,$datalist);
		...
		foreach($dingdan as $value){
			...
			foreach($yuanarr as $y){
				if($value[0]==$y['dingdanhao']){
					...
					$itemx['riqi']=$value[3];
					//得到规格型号
					$itemx['xinghao']=$y['xinghao'];
					//得到应发数量
					$itemx['yingfa']=$y['yingfa'];
					//得到已发数量
					$itemx['yifa']=$y['yifa'];
					//得到库存数量
					$itemx['kucun']=$y['kucun'];
					//得到备注
					$itemx['beizhu']=$y['beizhu'];
					$dingdanlist[]=$itemx;
				}
			}
		}
